...
$(function () {
  ...
  $('#comment-form').submit(function () {
    ...
    var appendComment = function (comment) {
      ...
      if ($(".reply2view").length > 0) {
        $.ajax({
          ...
          type: 'get',
          error: function (request) {
            $.NotificationApp.send("警告!", "回复可见内容需要刷新页面后可见！", "top-center", "rgba(0,0,0,0.2)", "warning");
          },
          success: function (data) {
            if ($(data).find('.reply2view-ok').length > 0) {
              ...
              $(".reply2view").remove();
              $.NotificationApp.send("提示!", "回复可见内容已成功为您显示！", "top-center", "rgba(0,0,0,0.2)", "success");
            }
          }
        });
      }
    ...
    $.ajax({
      ...
      dataType: 'json',
      beforeSend: function () {
        form.find('#misubmit').addClass('loading').html('<span class="spinner-border spinner-border-sm mr-1" role="status" aria-hidden="true"></span>提交中...').attr('disabled', 'disabled');
      },
      complete: function () {
        form.find('#misubmit').removeClass('loading').html('提交').removeAttr('disabled');
      },
